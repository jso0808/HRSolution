package recruit;

public class RecruitDTO {
private String posNo;
private int PosNum;
private String pos;
private String PosStart;
private String posEnd;
private String PosMeans;
private String PosPro;
private String deptNo;
private String id;
private String posTitle;

private String ApNo;
private String ApName;
private String ApBirth;
private String ApDate;
private String ApInterview;
private String ApRoute;

private String ApResult1;
private String ApResult2;
private String ApResult3;
private String ApTel;

private String evNo;

private int evGrade1;
private int evGrade2;
private int evGrade3;
private String evReason;

private String idRe;
private String ReNo;
private String dept;
public String getDept() {
	return dept;
}
public void setDept(String dept) {
	this.dept = dept;
}
public String getIdRe() {
	return idRe;
}
public void setIdRe(String idRe) {
	this.idRe = idRe;
}
public String getReNo() {
	return ReNo;
}
public void setReNo(String reNo) {
	ReNo = reNo;
}
public String getEvNo() {
	return evNo;
}
public void setEvNo(String evNo) {
	this.evNo = evNo;
}
public int getEvGrade1() {
	return evGrade1;
}
public void setEvGrade1(int evGrade1) {
	this.evGrade1 = evGrade1;
}
public int getEvGrade2() {
	return evGrade2;
}
public void setEvGrade2(int evGrade2) {
	this.evGrade2 = evGrade2;
}
public int getEvGrade3() {
	return evGrade3;
}
public void setEvGrade3(int evGrade3) {
	this.evGrade3 = evGrade3;
}
public String getEvReason() {
	return evReason;
}
public void setEvReason(String evReason) {
	this.evReason = evReason;
}
public String getApNo() {
	return ApNo;
}
public void setApNo(String apNo) {
	ApNo = apNo;
}
public String getApName() {
	return ApName;
}
public void setApName(String apName) {
	ApName = apName;
}
public String getApBirth() {
	return ApBirth;
}
public void setApBirth(String apBirth) {
	ApBirth = apBirth;
}
public String getApDate() {
	return ApDate;
}
public void setApDate(String apDate) {
	ApDate = apDate;
}
public String getApInterview() {
	return ApInterview;
}
public void setApInterview(String apInterview) {
	ApInterview = apInterview;
}
public String getApRoute() {
	return ApRoute;
}
public void setApRoute(String apRoute) {
	ApRoute = apRoute;
}
public String getApResult1() {
	return ApResult1;
}
public void setApResult1(String apResult1) {
	ApResult1 = apResult1;
}
public String getApResult2() {
	return ApResult2;
}
public void setApResult2(String apResult2) {
	ApResult2 = apResult2;
}
public String getApResult3() {
	return ApResult3;
}
public void setApResult3(String apResult3) {
	ApResult3 = apResult3;
}
public String getApTel() {
	return ApTel;
}
public void setApTel(String apTel) {
	ApTel = apTel;
}
public String getPosNo() {
	return posNo;
}
public void setPosNo(String posNo) {
	this.posNo = posNo;
}
public int getPosNum() {
	return PosNum;
}
public void setPosNum(int posNum) {
	PosNum = posNum;
}
public String getPos() {
	return pos;
}
public void setPos(String pos) {
	this.pos = pos;
}
public String getPosStart() {
	return PosStart;
}
public void setPosStart(String posStart) {
	PosStart = posStart;
}
public String getPosEnd() {
	return posEnd;
}
public void setPosEnd(String posEnd) {
	this.posEnd = posEnd;
}
public String getPosMeans() {
	return PosMeans;
}
public void setPosMeans(String posMeans) {
	PosMeans = posMeans;
}
public String getPosPro() {
	return PosPro;
}
public void setPosPro(String posPro) {
	PosPro = posPro;
}
public String getDeptNo() {
	return deptNo;
}
public void setDeptNo(String deptNo) {
	this.deptNo = deptNo;
}
public String getId() {
	return id;
}
public void setId(String id) {
	this.id = id;
}
public String getPosTitle() {
	return posTitle;
}
public void setPosTitle(String posTitle) {
	this.posTitle = posTitle;
}
}
